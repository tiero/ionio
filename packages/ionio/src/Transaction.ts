import { address, AssetHash, confidential, Psbt, script, witnessStackToScriptWitness } from 'liquidjs-lib';
...
export interface TransactionInterface {
  ...
  withRecipient(addressOrScript: string | Buffer, amount: number, assetID: string): TransactionInterface;
  withOpReturn(hexChunks: string[], value: number, assetID: string): TransactionInterface;
...
export class Transaction implements TransactionInterface {
  ...
  withRecipient(addressOrScript: string | Buffer, value: number, assetID: string = this.network.assetHash): this {
  ...
  async unlock(signer?: IdentityProvider): Promise<this> {
    ...
    this.artifactFunction.require.forEach(async ({ type }) => {
      switch (type) {
        case RequirementType.Signature:
          if (!signer)
            throw new Error('contract requires signature but no IdentityProvider was provided');

          const signedPtxBase64 = await signer.signTransaction(this.psbt.toBase64());
        ...
        case RequirementType.DataSignature:
          if (!this.artifactFunction.functionInputs.some(p => p.type === PrimitiveType.DataSignature))
            throw new Error('contract requires data signature but no DataSignature was provided');
      }
    });
    ...
    this.psbt.finalizeInput(this.fundingUtxoIndex!, (_, input) => {
      return {
        ...
        finalScriptWitness: witnessStackToScriptWitness(witnessStack.concat([
